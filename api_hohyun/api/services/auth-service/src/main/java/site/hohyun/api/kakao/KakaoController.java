    ...
    public RedirectView kakaoCallback(
        ...
        if (code != null) {
            try {
                ...
                // 3. JWT 토큰 생성
                String userId = extractedUserInfo.get("kakao_id").toString();
                ...
                String redirectUrl = frontendUrl + "/login/callback?provider=kakao&error=" + URLEncoder.encode("인증 처리 중 오류가 발생했습니다.", StandardCharsets.UTF_8);
                return new RedirectView(redirectUrl);
            }
        } else if (error != null) {
